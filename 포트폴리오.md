...
<br>
**데이터를 이해하고, 문제에 맞는 AI 기술을 직접 구현해왔습니다.**
수학과 데이터 분석을 기반으로 Generative Recommendation, Computer Vision, Regression Analysis 등 다양한 프로젝트를 경험했습니다. 새로운 기술을 빠르게 익히고 직접 실험하며, 문제의 특성에 맞는 방법을 찾아 결과로 연결하는 과정을 중요하게 생각합니다.

# 📌 Projects Overview
| 프로젝트 | 기간 | 역할 | 기술 스택 |
|---|---|---|---|
| Generative Recommendation Research | 2025.07 ~ 현재 | 학부연구생 | Python, PyTorch, Generative Recommendation |
...
## 1. Generative Recommendation Research 🔬

**한양대학교 데이터마이닝 연구실 학부연구생**

Generative Recommendation 분야의 선행 연구를 분석하고, 기존 모델의 재현과 평가 환경 구축을 수행하고 있습니다. 초기에는 Sequential Recommendation 관련 논문을 조사·발표하고 GRAM을 비롯한 Baseline 모델을 재현했습니다. 이후 Baseline별 평가 방식의 차이를 분석하고 Candidate Sampling에서 Full Ranking으로 평가 방법을 통일했습니다.

현재는 **item embedding의 의미적 구조를 Graph로 분석해 Adaptive한 Item Identifier를 구성하고, 사용자 행동 정보를 Generation 과정에 활용하는 방법**을 다루고 있습니다. GID(Graph-based Identifier)와 CCD(Collaborative Constrained Decoding)의 아이디어를 검증하기 위해 다양한 조건과 Case에 대한 실험을 수행하며 모델의 성능과 특성을 분석하고 있습니다.
...
- **이미지 전처리:** U-2-Net과 Mask R-CNN을 활용해 이미지의 배경 제거와 객체 분할을 수행하는 전처리 Pipeline을 구축했습니다. 또한 Data Augmentation을 적용해 모델의 일반화 성능을 높이고자 했습니다.
...
한국지능정보사회진흥원(NIA) Data Creator Camp에서 **최우수상(2위)**을 수상했습니다. 
...
**한양대학교 「Tech for Impact」 수업 프로젝트 (KAKAO Impact & Refeely 협업)**
...
- **Frontend 및 Game Engine 개발:** Phaser3와 JavaScript를 활용해 폐기물 분류 게임, 재활용 전처리 미션, 퀴즈 등 핵심 게임 로직을 구현하고 사용자 경험을 고려한 UI/UX를 개발했습니다.
