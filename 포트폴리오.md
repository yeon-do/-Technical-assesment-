# 📜 Technical assessment (박도연)
Portfolio of yeon-do


# 📌 Projects Overview
| 프로젝트 | 기간 | 역할 | 기술 스택 |
|---|---|---|---|
| 한양대학교 DataminingLab 학부연구생 | 2025.07 ~ 현재 | 학부연구생 | Python, PyTorch, Generative Recommendation |
| 응용회귀분석 | 2025.09 ~ 2025.12 | Data Analyst | R, Statistics |
| 패션 스타일 이미지 Classification & Preference Prediction | 2024.09 ~ 2024.11 | Model Architect | Python, PyTorch |
...
Generative Recommendation 분야의 선행 연구를 분석하고, 기존 모델의 재현과 평가 환경 구축을 수행하고 있습니다. 초기에는 Sequential Recommendation 및 Generative Recommendation 관련 논문을 조사·발표하며 기존 연구의 방법론과 실험 구성을 파악했고, 이후 GRAM을 비롯한 baseline 모델을 직접 재현하여 후속 실험을 위한 환경을 구축했습니다. 또한 baseline마다 사용하고 있는 데이터와 평가 방식의 차이를 비교·분석하고, 서로 다른 평가 설정으로 인해 발생할 수 있는 성능 비교의 한계를 확인했습니다. 이를 바탕으로 Candidate Sampling 방식에서 Full Ranking 방식으로 평가 방법을 통일하여 동일한 조건에서 모델을 비교할 수 있는 실험 환경을 구성했습니다.

현재는 item embedding이 갖는 의미적 구조와 사용자 행동 정보를 Generative Recommendation에 활용하는 방법을 연구하고 있습니다. 구체적으로는 item embedding 간의 의미적 유사성을 Graph 구조로 표현하고, Graph의 community를 분석하여 각 item의 의미적 특성을 반영할 수 있는 adaptive한 item identifier를 구성하는 아이디어를 검증하고 있습니다. 기존의 고정된 codebook이나 동일한 branching 구조를 사용하는 방식과 달리, item embedding 공간의 의미적 밀도에 따라 identifier의 구조를 유연하게 구성함으로써 유사한 item 간의 semantic overlap을 줄이는 방향입니다. 또한 사용자의 과거 행동에서 나타나는 collaborative signal을 활용하여 autoregressive generation 과정에서 다음 item을 예측할 때 관련 사용자들의 행동 정보를 함께 반영하는 방법도 연구하고 있습니다.

현재는 이러한 아이디어가 실제 recommendation 성능과 generation 과정에 어떤 영향을 미치는지 확인하기 위해 다양한 조건과 case를 설정하여 반복적으로 실험하고 있습니다. 각 case에서 성능 변화를 비교하는 것뿐만 아니라, 특정 조건에서 어떤 현상이 나타나는지 분석하며 제안 아이디어가 효과를 보이는 상황과 그렇지 않은 상황을 구분하고 있습니다. 이를 통해 단순히 최종 성능을 비교하는 데 그치지 않고, 각 구성 요소가 recommendation 결과에 미치는 영향을 단계적으로 확인하며 연구 방향을 구체화하고 있습니다.
...
한국지능정보사회진흥원(NIA) Data Creator Camp에서 **최우수상(2위)**을 수상했습니다. Computer Vision과 Recommendation을 결합한 접근 방식과 모델 구성의 논리성을 바탕으로 프로젝트의 완성도를 인정받았습니다.
